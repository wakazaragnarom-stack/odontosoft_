...
async def request_logging(request: Request, call_next):
    ...
    logger.info("%s %s -> %s (%.4fs)", request.method, request.url.path, response.status_code, elapsed)
    return response
...
async def sqlalchemy_exception_handler(request: Request, exc: SQLAlchemyError):
    ...
    return JSONResponse(
        status_code=500,
        content={"detail": "Error al procesar la operación de base de datos"},
    )
...
# Los módulos existentes son routers aunque históricamente vivan en la raíz.
import auth
import usuarios
import pacientes
import odontologos
