...
function Clinical({ patient, notify }: any) {
  const [note, setNote] = useState(''); const [tooth, setTooth] = useState(11); const [condition, setCondition] = useState<PatientRecord['odontogram'][number]['status']>('healthy');
  ...
  return <div className="space-y-5"><div><h2 className="text-xl font-black">Historia clínica: {patient.name}</h2><p className="text-sm text-slate-500">Odontograma + anamnesis + evolución en un único expediente.</p></div><div className="grid xl:grid-cols-2 gap-5"><section className="bg-white border border-slate-200 rounded-2xl p-5"><h3 className="font-black mb-4">Odontograma</h3><div className="flex gap-2"><input type="number" value={tooth} onChange={e=>setTooth(Number(e.target.value))} className="w-24 border rounded-xl px-3 py-2"/><select value={condition} onChange={e=>setCondition(e.target.value as PatientRecord['odontogram'][number]['status'])} className="border rounded-xl px-3 py-2 flex-1"><option value="healthy">Sano</option><option value="caries">Caries</option><option value="filled">Obturado</option><option value="crown">Corona</option><option value="missing">Ausente</option><option value="implant">Implante</option><option value="root_canal">Endodoncia</option></select><button onClick={()=>{updateToothStatus(patient.id,tooth,condition);notify(`Diente ${tooth} actualizado`)}} className="px-4 rounded-xl bg-slate-900 text-white text-sm font-bold">Guardar</button></div><div className="mt-5 grid grid-cols-4 sm:grid-cols-8 gap-2">{patient.odontogram.map((item:any)=><span key={item.toothNumber} className="px-2 py-2 bg-slate-50 rounded-lg text-center text-xs border">{item.toothNumber}<br/><strong>{item.status}</strong></span>)}</div></section><section className="bg-white border border-slate-200 rounded-2xl p-5"><h3 className="font-black mb-4">Evolución clínica</h3><textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Nota de atención..." className="w-full min-h-28 border rounded-xl p-3 text-sm"/><button onClick={saveEvolution} className="mt-3 inline-flex items-center gap-2 bg-sky-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold"><FileText className="w-4 h-4"/>Guardar evolución</button><div className="mt-5 space-y-3">{patient.clinicalEvolution.slice(0,8).map((entry:any)=><div key={entry.id} className="border-l-4 border-sky-200 pl-3"><div className="text-xs font-bold text-slate-500">{entry.date} · {entry.dentistName}</div><div className="text-sm mt-1">{entry.notes}</div></div>)}</div></section></div></div>;
}

function Treatments({ treatments, tenantId, notify }: any) {
  const [form, setForm] = useState({ nombre:'', descripcion:'', costo:'', duracion_estimada:'30 min' });
  return <div className="space-y-4"><h2 className="text-xl font-black">Tratamientos</h2><div className="bg-white border border-slate-200 rounded-2xl p-5"><div className="grid md:grid-cols-4 gap-3">{Object.entries(form).map(([key,value])=><input key={key} value={value as string} onChange={e=>setForm({...form,[key]:e.target.value})} placeholder={key} className="border rounded-xl px-3 py-2 text-sm"/>)}</div><button onClick={()=>{if(!form.nombre.trim())return;createTratamiento({tenantId,nombre:form.nombre.trim(),descripcion:form.descripcion,costo:Number(form.costo)||0,duracion_estimada:form.duracion_estimada});setForm({nombre:'',descripcion:'',costo:'',duracion_estimada:'30 min'});notify('Tratamiento creado')}} className="mt-3 bg-sky-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold">Crear tratamiento</button></div><div className="grid lg:grid-cols-3 gap-3">{treatments.map((t:any)=><div key={t.id_tratamiento} className="bg-white border border-slate-200 rounded-2xl p-4"><div className="font-black">{t.nombre}</div><div className="text-xs text-slate-500 mt-1">{t.descripcion}</div><div className="font-bold mt-3">${Number(t.costo).toLocaleString()}</div></div>)}</div></div>;
}

function Billing({ appointments, notify }: any) {
  const [selected, setSelected] = useState(appointments[0]?.id || ''); const [method, setMethod] = useState<Pago['metodo_pago']>('Efectivo'); const [ref,setRef]=useState('');
  return <div className="space-y-4"><div><h2 className="text-xl font-black">Pagos y facturas</h2><p className="text-sm text-slate-500">Registro operativo conectado al expediente de la cita.</p></div><div className="bg-white border border-slate-200 rounded-2xl p-5"><div className="grid md:grid-cols-3 gap-3"><select value={selected} onChange={e=>setSelected(e.target.value)} className="border rounded-xl px-3 py-2 text-sm">{appointments.map((a:any)=><option key={a.id} value={a.id}>{a.patientName} · {a.date} · ${a.price}</option>)}</select><select value={method} onChange={e=>setMethod(e.target.value as Pago['metodo_pago'])} className="border rounded-xl px-3 py-2 text-sm"><option>Efectivo</option><option>Tarjeta de Crédito/Débito</option><option>Transferencia Bancaria</option><option>MercadoPago / PSE</option></select><input value={ref} onChange={e=>setRef(e.target.value)} placeholder="Referencia" className="border rounded-xl px-3 py-2 text-sm"/></div><button disabled={!selected} onClick={()=>{registrarPagoCita(selected,method,ref.trim());notify('Pago registrado y factura generada')}} className="mt-3 bg-emerald-600 disabled:opacity-50 text-white px-4 py-2.5 rounded-xl text-sm font-bold">Registrar pago</button></div></div>;
}

function Suppliers({ suppliers, tenantId, notify }: any) {
  const [empresa,setEmpresa]=useState(''); const [contacto,setContacto]=useState(''); const [telefono,setTelefono]=useState('');
  return <div className="space-y-4"><h2 className="text-xl font-black">Proveedores</h2><div className="bg-white border border-slate-200 rounded-2xl p-5"><div className="grid md:grid-cols-3 gap-3"><input value={empresa} onChange={e=>setEmpresa(e.target.value)} placeholder="Empresa" className="border rounded-xl px-3 py-2 text-sm"/><input value={contacto} onChange={e=>setContacto(e.target.value)} placeholder="Contacto" className="border rounded-xl px-3 py-2 text-sm"/><input value={telefono} onChange={e=>setTelefono(e.target.value)} placeholder="Teléfono" className="border rounded-xl px-3 py-2 text-sm"/></div><button onClick={()=>{if(!empresa.trim())return;createProveedor({tenantId,empresa:empresa.trim(),contacto_asesor:contacto.trim(),telefono:telefono.trim(),suministro:'',estado_convenio:'Vigente'});setEmpresa('');setContacto('');setTelefono('');notify('Proveedor creado')}} className="mt-3 bg-sky-600 text-white px-4 py-2.5 rounded-xl text-sm font-bold">Crear proveedor</button></div><div className="grid lg:grid-cols-3 gap-3">{suppliers.map((s:any)=><div key={s.id} className="bg-white border border-slate-200 rounded-2xl p-4"><div className="font-black">{s.empresa}</div><div className="text-xs text-slate-500 mt-1">{s.contacto_asesor} · {s.telefono}</div><div className="mt-2 text-xs font-bold">{s.estado_convenio}</div></div>)}</div></div>;
}

function NewAppointment({ patients, dentists, treatments, tenantId, onClose, notify }: any) {
  const [patientId,setPatientId]=useState(patients[0]?.id||''); const [dentistId,setDentistId]=useState(dentists[0]?.id||''); const [treatmentId,setTreatmentId]=useState(treatments[0]?.id_tratamiento||''); const [date,setDate]=useState(new Date().toISOString().slice(0,10)); const [time,setTime]=useState('09:00');
  const patient=patients.find((p:any)=>p.id===patientId); const dentist=dentists.find((d:any)=>d.id===dentistId); const treatment=treatments.find((t:any)=>t.id_tratamiento===treatmentId);
  return <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50"><div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl p-5"><div className="flex items-center justify-between"><h3 className="text-lg font-black">Nueva cita</h3><button onClick={onClose}><X className="w-5 h-5"/></button></div><div className="grid md:grid-cols-2 gap-3 mt-4"><select value={patientId} onChange={e=>setPatientId(e.target.value)} className="border rounded-xl px-3 py-2 text-sm">{patients.map((p:any)=><option key={p.id} value={p.id}>{p.name}</option>)}</select><select value={dentistId} onChange={e=>setDentistId(e.target.value)} className="border rounded-xl px-3 py-2 text-sm">{dentists.map((d:any)=><option key={d.id} value={d.id}>{d.name}</option>)}</select><select value={treatmentId} onChange={e=>setTreatmentId(e.target.value)} className="border rounded-xl px-3 py-2 text-sm md:col-span-2">{treatments.map((t:any)=><option key={t.id_tratamiento} value={t.id_tratamiento}>{t.nombre}</option>)}</select><input type="date" value={date} onChange={e=>setDate(e.target.value)} className="border rounded-xl px-3 py-2 text-sm"/><input type="time" value={time} onChange={e=>setTime(e.target.value)} className="border rounded-xl px-3 py-2 text-sm"/></div><button disabled={!patient||!dentist||!treatment} onClick={()=>{bookAppointment({tenantId,dentistId,dentistName:dentist.name,dentistEmail:dentist.email,patientId,patientName:patient.name,patientEmail:patient.email,patientPhone:patient.phone,patientDocumento:patient.documento,treatment:treatment.nombre,tratamientosIds:[treatment.id_tratamiento],date,time,durationMinutes:30,price:Number(treatment.costo)||0});notify('Cita creada');onClose()}} className="mt-4 w-full bg-sky-600 disabled:opacity-50 text-white px-4 py-2.5 rounded-xl text-sm font-bold">Guardar cita</button></div></div>;
}
