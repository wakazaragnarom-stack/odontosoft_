...
import { addClinicalEvolutionNote, bookAppointment, createProveedor, createTratamiento, dispararRecordatorios24h, getSaaSMetrics, getStoreState, registrarPagoCita, subscribeToStore, updateAppointmentStatus, updateToothStatus } from './lib/store';
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 flex">
      <aside className="hidden md:flex w-64 bg-slate-950 text-white flex-col shrink-0">
        ...
        <div className="p-4 border-t border-white/10 text-xs text-slate-400">Tenant: {store.tenants[0]?.name || 'Sin clínica'}</div>
      </aside>
      ...
      <main className="flex-1 min-w-0">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 gap-4 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-1.5 bg-emerald-50 text-emerald-700 text-xs font-bold rounded-lg">Sistema activo</span>
          </div>
        </header>
